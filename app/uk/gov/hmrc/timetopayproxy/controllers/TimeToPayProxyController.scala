...
class TimeToPayProxyController @Inject() (
      ...
      withJsonBody[UpdatePlanRequest] { updatePlanRequest: UpdatePlanRequest =>
        {
          ...
          val result = for {
            validatedUpdatePlanRequest <-
              validateUpdateRequestMatchesQueryParams(
                customerReference,
                planId,
                updatePlanRequest
              )
            response <- timeToPayProxyService.updatePlan(
              validatedUpdatePlanRequest
            )
          } yield response
  ...
  private def validateUpdateRequestMatchesQueryParams(
    ...
    (updatePlanRequest.customerReference, updatePlanRequest.planId) match {
      case (CustomerReference(cr), PlanId(pid))
          if (cr.trim == customerReference) && (pid.trim == planId) =>
        TtppEnvelope(updatePlanRequest)
      case _ =>
        TtppEnvelope(
          ValidationError(queryParameterNotMatchingPayload)
            .asLeft[UpdatePlanRequest]
        )
    }
  }
  ...
  private def generateReadableMessageFromError(
    ...
    val detailedMessageMaybe = for {
      ...
      case m if m.startsWith("error.expected.date.isoformat") =>
        "Date format should be correctly provided"
      case m if m.startsWith("error.expected.validenumvalue") =>
        "Valid enum value should be provided"
      case _ => ""
    }
