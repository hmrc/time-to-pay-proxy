...
final case class TtpFullAmendInformativeError(
  ...
  internalErrors: List[TtpFullAmendInternalError],
...
case class TtpFullAmendInternalError(message: String)

object TtpFullAmendInternalError {
  implicit val format: OFormat[TtpFullAmendInternalError] = Json.format[TtpFullAmendInternalError]
}
